...
void sl_bt_on_event(sl_bt_msg_t *evt) {
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    // --------------------------------------------------------------
    // This event indicates the device has started & the radio is ready. Do not call any stack cmd before receiving this boot event!
    case sl_bt_evt_system_boot_id:
      ...
      // Check the report button state, then update the characteristic and send notification.
      sc = update_report_button_characteristic();
      app_log_status_error(sc);
    ...
    // --------------------------------------------------------------
    // This event indicates that the value of an attribute in the local GATT database was changed by a remote GATT client.
    case sl_bt_evt_gatt_server_attribute_value_id:
      ...
      if (gattdb_led_control == evt->data.evt_gatt_server_characteristic_status.characteristic) {
        ...
        sc = sl_bt_gatt_server_read_attribute_value(gattdb_led_control, 0, sizeof(data_recv), &data_recv_len, &data_recv);
        (void)data_recv_len;
        app_log_status_error(sc);
    ...
    // --------------------------------------------------------------
    // This event occurs when the remote device enabled or disabled the notification.
    case sl_bt_evt_gatt_server_characteristic_status_id:

      if (gattdb_report_button == evt->data.evt_gatt_server_characteristic_status.characteristic) {

    	  // A local Client Characteristic Configuration descriptor was changed in the gattdb_report_button characteristic.
        if (evt->data.evt_gatt_server_characteristic_status.client_config_flags & sl_bt_gatt_notification) {
          // The client just enabled the notification. Send notification of the current button state stored in the local GATT table.
...
static sl_status_t update_report_button_characteristic(void) {
  ...
  sc = sl_bt_gatt_server_write_attribute_value(gattdb_report_button, 0, sizeof(data_send), &data_send);
 ...
 * Sends notification of the Report Button characteristic.
 *
 * Reads the current button state from the local GATT database and sends it as a notification.
 ************************************************************************************************************************************/
static sl_status_t send_report_button_notification(void) {
  ...
  sc = sl_bt_gatt_server_read_attribute_value(gattdb_report_button, 0, sizeof(data_send), &data_len, &data_send);
  ...
  sc = sl_bt_gatt_server_notify_all(gattdb_report_button, sizeof(data_send), &data_send);
